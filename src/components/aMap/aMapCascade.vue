<template>
    <div>
        <!-- 下拉选择框 -->
        <div @click="changeCascadeState" class="pull-down-input" :class="{'disabledClass': isDisabled}" :title="street">
            <span>{{street}}</span>
        ...
        <div class="pull-down-total-div absolute-pull" v-if="theOpenState">
            <div ref="childsDiv" class="pull-down-childs-div max-height300 box-shadow">
                <div v-for="(item,index) of data"
                     class="pull-down-child"
                     @click="changeCascadeChildState(index)"
                     :title="item.name"
                     :key="index">
                    <span>{{item.name}}</span>
...
<script>
import casChild from '@/components/aMap/cascadeChild.vue'
import AMap from 'AMap' // 在页面中引入高德地图
export default{
  name: 'aMap',
  props: ['aMapOnChange', 'isDisabled'],
  data: function () {
    return {
  ...
  methods: {
    ...
    setStreetAndStreets (s) {
      if (s) {
        this.street = s
        this.streets = s.split('/')
      }
    },
    reset () {
      this.street = '街道选择框'
      this.streets = ['中国']
      this.theOpenState = false
      this.theOpenChild = false
    }
  }
}
