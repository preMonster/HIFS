<template>
    <div>
        <ModuleTitle :iconName="'paper-airplane'" :moduleName="'违法基础信息'"></ModuleTitle>
        ...
        <IllegalBaseInfoForm ref="modal"
                  :modalData='modalData'
                  :save="save"></IllegalBaseInfoForm>
    </div>
</template>
...
import IllegalBaseInfoForm from '@/components/illegalBaseInfo/IllegalBaseInfoForm'
export default {
  name: 'illegalBaseInfo',
  data: function () {
    return {
      modalData: [0, '新增', {}],
      chooseRows: [],
      tableAttr: {
        url: 'api/hifs/illegal/getIllegalsByPage',
        params: {},
          ...
          },
          {
            ...
            render (h, params) {
              return h(
                'p',
                {},
                params.row.punish ? params.row.punish : '-'
              )
            },
            disabledSearch: true
          },
      ...
      if (rows.length === 0) {
        ...
        this.$Modal.error({
          ...
          onOk: function () {
            ...
            param.append('illegals', JSON.stringify(rows))
            ajax.post({
              url: 'api/hifs/illegal/deleteIllegal',
              param: param,
              success: res => {
                let param = new URLSearchParams()
