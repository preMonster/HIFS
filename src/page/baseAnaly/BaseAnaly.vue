<template>
    <div>
        <ModuleTitle :iconName="'paper-airplane'" :moduleName="'基础路段统计分析'"></ModuleTitle>
        <Card class='module-card'>
          <div class="toolbar">
            <Cascader @on-change="init" :data="cascaderData" v-model="value1" class="toolbar-formItem"></Cascader>
            <Select @on-change="init" class="toolbar-formItem" v-model="year">
                  <Option value="">--</Option>
...
export default {
  ...
  methods: {
    ...
    pieInit (value) {
      ...
      ajax.get({
        url: 'api/hifs/record/getPieRecords?monitorId=' + monitorId + '&year=' + year,
        param: {},
        success: res => {
          let lArr = []
